...
# Relative Difference MMD from Mu
# COnvert from matrix to dataframe
...
gg<- ggplot(df, aes(mu, sigma, fill= z)) + 
  ...
  theme(legend.position="top", legend.title = element_blank(),
        ...
        legend.box.spacing = unit(.1,"inch"))
gg
save_plot(paste("figure/SF", fig_num, "/SF", fig_num, "_a2 mmd rdiff.tiff",sep=""),
...
gg
save_plot(paste("figure/SF", fig_num, "/SF", fig_num, "_a2 mmd rdiff_vs_mus.tiff",sep=""),
          gg, base_height = 2.2, base_asp = 3, base_width = 2.2, dpi = 600) 
# Plot slope of heatmap by row
gg <- ggplot(data = grid_slopes$df_row, aes(x = row_vals, y = slope)) +
  geom_line(size=0.5) + geom_point(size=0.4) + 
  ...
  theme_classic(base_size=8)
gg
save_plot(paste("figure/SF", fig_num, "/SF", fig_num, "_a2 mmd rdiff_vs_sigmas.tiff",sep=""),
          gg, base_height = 2.2, base_asp = 3, base_width = 2.2, dpi = 600) 
...
#         legend.box.spacing = unit(.1,"inch"))
# gg
# save_plot(paste("figure/SF", fig_num, "/SF", fig_num, "_a3 mmd error rate 2D.tiff",sep=""),
#           gg, ncol = 1, nrow = 1, base_height = 2.2,
#           base_asp = 3, base_width = 2, dpi = 600) 
# # Plot slope of heatmap by column
# gg <- ggplot(data = grid_slopes$df_col, aes(x = col_vals, y = slope)) +
#   geom_line(size=0.5) + geom_point(size=0.4) + 
#   geom_ribbon(aes(ymin = grid_slopes$df_col$slope_95L, ymax = grid_slopes$df_col$slope_95U), alpha=0.2) +
#   xlab(expression(mu)) + ylab(expression(paste("Slope By Column  (ER~",sigma,")"))) + theme_classic(base_size=8)
# gg
# save_plot(paste("figure/SF", fig_num, "/SF", fig_num, "_a3 mmd error rate 2D_vs_mus.tiff",sep=""),
#           gg, base_height = 2.2, base_asp = 3, base_width = 2.2, dpi = 600) 
# # Plot slope of heatmap by row
# gg <- ggplot(data = grid_slopes$df_row, aes(x = row_vals, y = slope)) +
#   geom_line(size=0.5) + geom_point(size=0.4) + 
#   geom_ribbon(aes(ymin = grid_slopes$df_row$slope_95L, ymax = grid_slopes$df_row$slope_95U), alpha=0.2) +
#   xlab(expression(sigma)) + ylab(expression(paste("Slope By Row  (ER ~ ",abs(~mu*phantom(.))*phantom(.)," )"))) + theme_classic(base_size=8)
# gg
# save_plot(paste("figure/SF", fig_num, "/SF", fig_num, "_a3 mmd error rate 2D_vs_sigmas.tiff",sep=""),
#           gg, base_height = 2.2, base_asp = 3, base_width = 2.2, dpi = 600) 
