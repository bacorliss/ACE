...
calc_stats <- function(df, out_path, csv_name, rel_delta) {
  ...
  for (n in seq(1, dim(df)[1]) ) {
    ...
    df_stat$bf[n] <- row_bayesf_2s(xa, xb, nullInterval = c(-rel_delta*mean(xa)/pooled_var, 
                                                            rel_delta*mean(xa)/pooled_var))[1]
    ...
    df_stat$p_equiv[n] <- row_tost_2s(xa, xb, eqbounds = c(- rel_delta*mean(xa), 
                                                            + rel_delta*mean(xa)), 
                                      conf.level = 1 - alpha_dm)
    df_stat$cd[n] <- row_cohend(xb, xa)
    
    df_stat$p_2nd[n] <- row_sgpv(xa, xb, -rel_delta * mean(xa), rel_delta * mean(xa))
  ...
  print(gg)
  save_plot(paste(out_path, "/credint_", csv_name, ".tiff",sep=""),
  ...
  gg = ggplot(data = df_stat, aes(x=x, y = rxbar_dm)) +
    ...
    print(gg)
  save_plot(paste(out_path, "/rcredint_", csv_name, ".tiff",sep=""),
  ...
  write.csv(t(df_stat), paste(out_path,"/", str_replace(csv_name,'.csv','_t.csv'),sep=""))
  # browser();
...
raw_chol_null <- read_excel("mdm_t/applied_examples.xlsx", sheet = "Chol Null", skip = 1)
chol_null <- raw_chol_null[-seq(min(which(is.na(raw_chol_null$sX))), dim(raw_chol_null)[1], 1),]
df_stat_chol_null <- calc_stats(chol_null, fig_path,"chol_null.csv", rel_delta = 0.3)
df_stat_chol_null


# Total CHolesterol, crit Results
raw_chol_crit <- read_excel("mdm_t/applied_examples.xlsx", sheet = "Chol Crit", skip=1)
chol_crit <- raw_chol_crit[-seq(min(which(is.na(raw_chol_crit$sX))), dim(raw_chol_crit)[1], 1),]
df_stat_chol_crit <- calc_stats(chol_crit, fig_path,"chol_crit.csv", rel_delta = 0.3)



# Plaque Size, Null Results
raw_plaq_null <- read_excel("mdm_t/applied_examples.xlsx", sheet = "Plaque Null", skip=1)
plaq_null <- raw_plaq_null[-seq(min(which(is.na(raw_plaq_null$sX))), dim(raw_plaq_null)[1], 1),]
df_stat_plaq_null <- calc_stats(plaq_null, fig_path,"plaq_null.csv", rel_delta = 0.4)


# Plaque Size, Null Results
raw_plaq_crit <- read_excel("mdm_t/applied_examples.xlsx", sheet = "Plaque Crit", skip=1)
plaq_crit <- raw_plaq_crit[-seq(min(which(is.na(raw_plaq_crit$sX))), dim(raw_plaq_crit)[1], 1),]
df_stat_plaq_crit <- calc_stats(plaq_crit, fig_path,"plaq_crit.csv", rel_delta = 0.4)
