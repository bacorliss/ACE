...
plot_experiments <- function(xa1,xb1,xa2,xb2, fig_num, fig_path, base_name, 
  ...
  save_plot(paste(fig_path, "F",fig_num, "_", base_name,"_E1", ".tiff", sep = ""),
            gg1, dpi = 600, base_height = 1.2, base_width = 1.25)
  
  # Experiment 2
  df <- tibble(group = as.factor(c(rep("A",length(xa2)),rep("B",length(xb2)))),y = c(xa2,xb2))
  df_stats <- df %>% group_by(group) %>% summarise(mean = mean(y),sd = sd(y))
  gg2 <- ggplot(data=df, aes(x=group, y=y)) +
    geom_jitter(width = 0.2, size=0.3, color = grp_color[2]) + 
    geom_point(data = df_stats, aes(x=group, y=mean), pch=3) +
    geom_blank(data=blank_df, aes(x=group, y=y)) + coord_cartesian(ylim = ylims) +
    theme_classic(base_size=7) + xlab("Exp 2") + ylab("Data") +
    geom_errorbar(data = df_stats, aes(x=group,ymin=mean-sd, ymax=mean+sd, y=mean), 
                  width=0.4) +
  ...
  save_plot(paste(fig_path, "F", fig_num, "_", base_name,"_E2", ".tiff", sep = ""),
            gg2, dpi = 600, base_height = 1.2, base_width = 1.25)
  ...
  if (data_scale=="relative") {
    ...
    gg3 <- ggplot(data = df_sample, aes(x=group, y=y)) +
      geom_violin(aes(fill=group), scale = "width", alpha=0.7) + theme_classic(base_size=7) + 
  ...
  save_plot(paste(fig_path, "F", fig_num, "_", base_name,"_D", ".tiff", sep = ""),
            gg3, dpi = 600, base_height = 1.2, base_width = 1.25)
  print(t(dfo))
  # browser();
}
...
# Normalize points
xa1 <- (a1-mean(a1))/sd(a1)*sqrt(1.3^2/2) + 10
xb1 <- (b1-mean(b1))/sd(b1)*sqrt(1.3^2/2) + 10
xa2 <- (a2-mean(a2))/sd(a2)*sqrt(1.3^2/2) + 10
xb2 <- (b2-mean(b2))/sd(b2)*sqrt(1.3^2/2) + 11.5
...
plot_experiments(xa1,xb1,xa2,xb2,fig_num=fig_num, fig_path=fig_path, base_name="mu_box", 
                 ylims = c(8,14), dm_ylims = c(-3.7,4), dm_ybreaks=seq(-3,3,3))
...
#-------------------------------------------------------------------------------
# Normalize points
xa2 <- (a2-mean(a2))/sd(a2)*sqrt(1^2/2)/0.5e5 + 1e-4
xb2 <- (b2-mean(b2))/sd(b2)*sqrt(1^2/2)/0.5e5 + 1e-4
# Plot experiments and Difference of Means
plot_experiments(xa1,xb1,xa2,xb2, fig_num=fig_num, fig_path=fig_path, dm_ylims = dm_ylims,
