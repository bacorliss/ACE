...
#------------------------------------------------------------------------------
# COnvert from matrix to dataframe
df <- cbind(sigma = sigmas, as_tibble(df_results$mean_diff_mmd_mu_vsigma)) %>% gather(mu, z, -sigma)
df$mu <- as.numeric(df$mu)
df$sigma <- as.numeric(df$sigma)
grid_slopes <- slopes_by_rowcol(df_results$mean_diff_mmd_mu_vsigma, sigmas, mus)
# Plot heatmap
gg<- ggplot(df, aes(mu, sigma, fill= z)) + 
  geom_tile()+ 
...
# COnvert from matrix to dataframe
df <- cbind(sigma = sigmas, as_tibble(df_results$mean_diff_mmd_mu_vmu)) %>% gather(mu, z, -sigma)
df$mu <- as.numeric(df$mu)
df$sigma <- as.numeric(df$sigma)
grid_slopes <- slopes_by_rowcol(df_results$mean_diff_mmd_mu_vmu, sigmas, mus)
# Plot heatmap
gg <- ggplot(df, aes(mu, sigma, fill = z)) + 
  geom_tile()+ 
  scale_x_continuous(expand=c(0,0)) + 
