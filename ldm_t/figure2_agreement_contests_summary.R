...
#-------------------------------------------------------------------------------
dfs_unscaled <- c(df_unscaled_crit)
...
# Export csv
rownames(scale_scores) <- attr(df_unscaled_crit[[1]]$df_es, "varnames")
...
scale_score_norm <- sapply(scale_norm_ind, function(ind,len) 
  ifelse(1:len == ind, TRUE,FALSE), length(attr(df_unscaled_crit[[1]]$df_es, "varnames")))
...
png(paste(sum_fig_path, "/F", fig_num, "_disagreement_contest raw scale.png",sep=""),    
    width = 5.63*300/2, height = 3*300, res = 300, pointsize = 8)  
heatmap.2(zeroed_scale_scores, trace = "none", dendrogram = "none", key = FALSE,
          ...
          labRow =  sapply(attr(df_unscaled_crit[[1]]$df_es, "varnames"),
...
#-------------------------------------------------------------------------------
# Export summary stats for relative scale data
dfs_relative <- c(df_relative_crit)
...
# Export csv
rownames(rscale_scores) <- attr(df_relative_crit[[1]]$df_es,"varnames")
...
rscale_score_norm <- sapply(relative_norm_ind, function(ind,len) 
  ifelse(1:len == ind, TRUE,FALSE), length(attr(df_relative_crit[[1]]$df_es,"varnames_pretty")))
...
heatmap.2(zeroed_rscale_scores, trace = "none", dendrogram = "none", key = FALSE,
          ...
          col = my_palette,  Rowv=F, Colv=F, sepwidth=c(0,0),
          labRow =  sapply(attr(df_relative_crit[[1]]$df_es,"varnames_pretty"),
                           function(x) parse(text=x)),labCol = "",
