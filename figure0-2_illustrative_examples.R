...
plot_experiments <- function(xa1,xb1,xa2,xb2, fig_num, fig_path, base_name, scale = "absolute") {
  ...
  gg1 <- ggplot(data=df, aes(x=group, y=y)) +
    geom_jitter(width = 0.2, size=0.5, color = "gray50") + 
  ...
  save_plot(paste(fig_path, "F",fig_num, "_", base_name,"_E1", ".tiff", sep = ""),
            gg1, dpi = 600, base_height = 1.4, base_width = 1)
  ...
  save_plot(paste(fig_path, "F", fig_num, "_", base_name,"_E2", ".tiff", sep = ""),
            gg2, dpi = 600, base_height = 1.4, base_width = 1)
 ...
 # Exp 1 only
  ...
  save_plot(paste(fig_path, "F", fig_num, "_", base_name,"_Exp1", ".tiff", sep = ""),
            gg4, dpi = 600, base_height = 1, base_width = .5)
  print(t(dfo))
}
...
# Normalize points
xa1 <- (a1-mean(a1))/sd(a1)*sqrt(1^2/2) * .8 + 0
xb1 <- (b1-mean(b1))/sd(b1)*sqrt(1^2/2) * 9 + 1
xa1 <- (a1-mean(a1))/sd(a1)*sqrt(1^2/2) * .75 + 0
xb1 <- (b1-mean(b1))/sd(b1)*sqrt(1^2/2) * .75 + 0.5
