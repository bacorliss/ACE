...
scale_scores <- t(t(scale_means_from_0.5)/scale_means_from_0.5[cbind(scale_norm_ind,seq_along(scale_norm_ind))])
# Export csv
rownames(scale_scores) <- effect_size_dict[[4]]
# Get statistical significance
scale_scores_sig <- !sapply(dfs_scale, function(x) get(x)$df_plotted$is_mean_0.5) #& scale_scores >0
rownames(scale_scores_sig) <- effect_size_dict[[4]]


#https://stackoverflow.com/questions/45890845/highlight-subset-cells-from-heatmap-by-row-col-index
...
heatmap.2(scale_scores, trace = "none", dendrogram = "none", key = FALSE, Rowv=F, Colv=F,
          add.expr = {makeRects(scale_scores_sig)}, col = my_palette, 
          labRow =  sapply(effect_size_dict[[4]], function(x) parse(text=x)),labCol = "",
          cellnote=matrix(sapply(scale_scores,function(x) sprintf("%0.2+f",x)),nrow = dim(scale_scores)[1]),
          notecol="black",notecex=0.75,row_names_side="left")


write.table(scale_scores,scale_contest_path, sep=",")
write.table(scale_scores_sig,scale_contest_path, sep=",", append=TRUE)
...
rscale_scores <- t(t(rscale_means_from_0.5)/rscale_means_from_0.5[cbind(rscale_norm_ind,seq_along(rscale_norm_ind))])
# Export csv
rownames(rscale_scores) <- effect_size_dict[[2]]
# Get statistical significance
rscale_scores_sig <- !sapply(dfs_rscale, function(x) get(x)$df_plotted$is_mean_0.5)
rownames(rscale_scores_sig) <- effect_size_dict[[2]]

write.table(rscale_scores,rscale_contest_path, sep=",")
write.table(rscale_scores_sig,rscale_contest_path, sep=",",append=TRUE)
...
# 
# 
# mapply(function(v,i) v/v[i], far_means_from_0.5,norm_ind)
# 
# apply( ,1,function(v,i) v/v[i])
# 
# far_norm_means <- far_means_from_0.5/far_means_from_0.5[1]
# # Write to csv where resultrs from each DF is a column
# write.table()
# 
# dfs_near <- c(ls(pat="dfs_[4-6]a"),ls(pat="dfs_[4-6]b"))
# 
# 
# 
